...
// ✅ FIXED: Case-insensitive static file middleware for Render (Linux is case-sensitive)
// On Windows/Mac, "studentLogin.html" and "studentlogin.html" are the same — NOT on Linux!
// This middleware finds the correct file regardless of the case used in the URL.
app.use((req, res, next) => {
  const publicDir = path.join(__dirname, "public");
  const requestedPath = req.path;
 
  // Only handle file requests (not API routes)
  if (requestedPath.includes(".")) {
    ...
    try {
      ...
      if (matchedFile && matchedFile !== fileName) {
        // Redirect to the correctly-cased filename
        return res.redirect(301, "/" + matchedFile);
      }
    } catch (e) {
      // If directory read fails, just continue normally
    }
  }
